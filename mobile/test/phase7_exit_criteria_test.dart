...
class MockConnectionService extends ConnectionService {
  ...
  Future<EventPush> getEventDetails(String eventId) async {
    ...
    return EventPush(
      event: event,
      entry: entry.copyWith(state: EntryState.seen),
    );
  }
  ...
  Future<EventLogs> getEventLogs(String eventId, int offset, int limit) async {
    return EventLogs(
      ...
      lines: [
        ...
        LogLine(
          offset: 101,
          ts: DateTime.utc(2026, 9, 19, 12, 0, 1),
          text: 'Prepared statement built',
        ),
        LogLine(
          offset: 102,
          ts: DateTime.utc(2026, 9, 19, 12, 0, 2),
...
void main() {
  testWidgets(
    'Phase 7 exit criteria: summary -> details -> logs -> approve end-to-end',
    (tester) async {
